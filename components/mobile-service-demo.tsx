...
import { Bell, CalendarDays, Check, ChevronRight, CircleAlert, Clock3, Home, Inbox, LayoutGrid, MoreHorizontal, Plus, Search, SlidersHorizontal, Sparkles, Trash2, UserRound } from "lucide-react"
  ...
  return <Sheet open={open} onOpenChange={setOpen}>
    ...
    <SheetContent side="bottom" className="mx-auto max-h-[92dvh] max-w-[430px] rounded-t-3xl px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))]">
      ...
      <form className="flex flex-col gap-4 py-5" onSubmit={(e) => { e.preventDefault(); setOpen(false); toast.success("Pesanan berhasil ditambahkan", { description: "Data #SRV-1049 tersimpan sebagai pesanan baru." }) }}>
        ...
        <div className="flex flex-col gap-2"><Label htmlFor="date">Tanggal</Label><Input id="date" type="date" required /></div>
  ...
  return <main className="min-h-dvh bg-canvas md:px-8 md:py-10">
    <div className="mobile-shell mx-auto min-h-dvh overflow-hidden border-border bg-background shadow-2xl md:min-h-[calc(100dvh-5rem)] md:rounded-[2rem] md:border">
      ...
      <div className="flex flex-col gap-10 px-5 pb-28 pt-6">
        <section className="flex flex-col gap-5"><div className="flex items-end justify-between gap-4"><SectionTitle eyebrow="Design system" title="Satu palet, banyak layanan" description="Evergreen, putih mineral, dan charcoal dipakai konsisten untuk laundry, rental, serta sewa lapangan." /><Badge variant="outline">v1.0</Badge></div><div className="grid grid-cols-5 gap-2" aria-label="Palet warna"><div className="h-14 rounded-xl bg-primary" /><div className="h-14 rounded-xl bg-accent" /><div className="h-14 rounded-xl bg-foreground" /><div className="h-14 rounded-xl border bg-card" /><div className="h-14 rounded-xl bg-muted" /></div>
